...
RUN mkdir -p /Tailored
COPY ./ /Tailored/
WORKDIR /Tailored 
#VOLUME /model

RUN cp -rf /Tailored/.copernicusmarine /root

RUN cp -rf /Tailored/.copernicusmarine ~/
RUN bash /Tailored/miniconda/Miniconda3-py38_4.12.0-Linux-x86_64.sh -b -p /opt/miniconda 

RUN . /opt/miniconda/bin/activate && conda env update --name base --file /Tailored/latest.yml 
