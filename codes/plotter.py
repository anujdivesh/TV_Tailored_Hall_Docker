...
##CALCULAIONS##
def first_page(folder_tmp,location_name,target_lat,target_lon,tide_name,station_name):
...
def produce_report(folder_tmp, report_name,location_name,target_lat,target_lon,tide_name,station_name,out_folder):
    with PdfPages(out_folder+'/'+report_name) as pdf:
        #plotting First Page
        
        fig = first_page(folder_tmp,location_name,target_lat,target_lon,tide_name,station_name)
        pdf.savefig()
        df = create_table()
        ...
        for table in df_list:
            ...
            for _, row in table.iterrows():
                ...
                colors_in_column[2] =get_color_from_jet(row["Tp"],0,20)
                colors_in_column[4] =get_color_from_jet(row["Tm"],0,15)
